...
본 시스템은 두 가지 점수 출처를 사용하는 하이브리드 구조입니다.

**1. 기존 학습 데이터에 포함된 식당 → `data/restaurant_label_scores.csv`**
- Out-of-Fold(OOF) 예측 점수를 사용합니다.
- 각 식당은 자신이 학습에 포함되지 않은 fold의 모델로 예측되므로, 데이터 누수가 없습니다.
- 5-fold 교차검증 기준으로 매 식당은 정확히 1개 fold에서만 validation 대상이 됩니다.

**2. 신규 식당 (학습 후 추가된 데이터) → `models/restaurant_recommender.joblib`**
- 전체 학습 데이터로 학습된 최종 모델이 실시간 추론합니다.
- `web_app.py`는 `--score-source` 옵션으로 CSV 우선 / 모델 우선을 선택할 수 있습니다.

이 설계는 **(1) 평가 시 데이터 누수 방지**와 **(2) 운영 시 신규 데이터 대응**을 동시에 만족하기 위함입니다.
...
| 항목 | 현재 상태 | 개선 방향 |
|------|---------|---------|
| 데이터 규모 | 477개 식당, 라벨별 positive 20~36개 수준 | 식당 수 확장 (목표 1,000개+) |
| 라벨 생성 | 키워드 사전 기반 → 사전 품질에 의존 | human-labeled 검증셋 확보 |
| 텍스트 표현 | 키워드 빈도 기반 점수 | 임베딩 기반(KLUE-BERT 등) 도입 |
| 지역 커버리지 | 강남·건대·잠실 3개 지역 | 서울 전역 확장 |
| couple_meal/drink 구분 | 공유 키워드 비중 높음 → 모델이 구별 어려움 | 카테고리 feature 활용 강화 |
